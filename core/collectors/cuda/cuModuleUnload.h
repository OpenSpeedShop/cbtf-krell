...
/**
 * Entry callback for cuModuleUnload().
 */
inline void cuModuleUnload_enter_callback(
    ...
    )
{
    ...
    if (debug)
    {
        printf("[CBTF/CUDA] cuModuleUnload_enter_callback()\n");
    }
#endif
...
/**
 * Exit callback for cuModuleUnload().
 */
inline void cuModuleUnload_exit_callback(
    const cuModuleUnload_params* const params
    )
{
#if !defined(NDEBUG)
    if (debug)
    {
        printf("[CBTF/CUDA] cuModuleUnload_exit_callback()\n");
    }
#endif
    ...
    CBTF_cuda_data data;
    data.type = UnloadedModule;
    
    CUDA_UnloadedModule* message =  &data.CBTF_cuda_data_u.unloaded_module;
