...
void cbtf_offline_send_dsos(TLS *tls)
{
    ...
    CBTF_EventSetSendToFile(&(tls->dso_header), "hwc", "cbtf-dsos");
...
void cbtf_offline_start_sampling(const char* in_arguments)
{
    ...
    tls->finished = 0;
    tls->started = 1;
    cbtf_hwc_service_start_sampling(NULL); /* offline processes parameters in runtime.*/
}
...
void cbtf_offline_stop_sampling(const char* in_arguments, const int finished)
{
    ...
    /* Stop sampling */
    cbtf_hwc_service_stop_sampling(NULL);
...
void cbtf_offline_finish()
{
    ...
    const char* hwc_event_param = getenv("CBTF_HWC_EVENT");
...
#if defined(CBTF_SERVICE_USE_FILEIO)
    CBTF_EventSetSendToFile(&local_header, "hwc", "cbtf-info");
