...
/** @file
 *
 * Declaration and definition of the IO collector's monitor runtime.
 *
 */
...
void cbtf_offline_send_dsos(TLS *tls)
{
    ...
    CBTF_EventSetSendToFile(&(tls->dso_header), "io", "cbtf-dsos");
...
void cbtf_offline_start_sampling(const char* in_arguments)
{
    ...
    char arguments[3 * sizeof(CBTF_io_start_sampling_args)];
...
#if defined(CBTF_SERVICE_USE_FILEIO)
    CBTF_EventSetSendToFile(&local_header, "io", "cbtf-info");
