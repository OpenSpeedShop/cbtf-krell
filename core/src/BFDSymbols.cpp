/*******************************************************************************
** Copyright (c) 2007-2015 The Krell Institue. All Rights Reserved.
**
...
/** @file
 *
 * Definition of the CBTF BFD symbol support functions.
...
bool BFDSymbols::is_debug_bfd_symbols_enabled =
    (getenv("CBTF_DEBUG_BFD_SYMBOLS") != NULL);
bool BFDSymbols::is_debug_bfd_symbols_details_enabled =
			     ...
			     bfd_vma obj_end_addr)
{
	...
	total_addrs++;
	if (range_in_this_obj.doesContain(aci->first)) {
// DEBUG
#ifndef NDEBUG
            if(is_debug_bfd_symbols_details_enabled) {
...
#endif
	    addrvec.push_back(aci->first.getValue());
	}
    }
...
// DEBUG
#ifndef NDEBUG
    if(is_debug_bfd_symbols_enabled) {
      std::cerr << "BFDSymbols::getFunctionSyms addrvec contatins " << addrvec.size()
	<< " addresses in this object out of " << total_addrs << std::endl;
    }
#endif
...
// DEBUG
	...
	<< " functions with data after examining " << total_addrs
...
int
BFDSymbols::getBFDFunctionStatements(const AddressBuffer& addrbuf,
...
// VERBOSE
	...
	if(is_debug_bfd_symbols_enabled) {
	    std::cerr << "Processing linked object "
	        << filename << " with address range " << lorange << std::endl;
	}
#endif
	...
	//for (unsigned ii = 0; ii < addrbuf->length; ii++) {
        for (aci = ac.begin(); aci != ac.end(); ++aci) {
	    total_addrs++;
	    if (lorange.doesContain(aci->first)) {
		addrvec.push_back(aci->first.getValue());
	    }
	}
	...
	int found_functions = getFunctionSyms(addrbuf,
					      lorange.getBegin().getValue(),
					      lorange.getEnd().getValue());
...
// VERBOSE
	...
	<< addresses_found << " out of buffer of " << total_addrs
	<< std::endl;
    }
#endif
...
void
BFDSymbols::getSymbols(const AddressBuffer& addrbuf,
		       const LinkedObjectEntry& lo, SymbolTable& stmap)
{
    ...
    AddressRange lorange = lo.getAddressRange();
	...
	//if (stmap.find(*si) != stmap.end()) {
	    //stmap_lorange = *si;
	    stmap_lorange = lorange;
// DEBUG
#ifndef NDEBUG
	    if(is_debug_bfd_symbols_enabled) {
	        std::cerr << "BFDSymbols::getSymbols: " << " stmap RANGE " << lorange << std::endl;
	    }
#endif
	//}
    }
...
//DEBUG
		    ...
		    stmap.addFunction(f->getFuncBegin(),
...
//DEBUG
		    ...
		    if (path.size() != 0) {
			stmap.addStatement(s_begin,s_end,path,line,col);
		    }
	    } //for
	}
    } // end for lorange
