 ...
 * performed by CBTF_Send(). Here the data is treated purely as a buffer of
 * bytes to be sent.
 *
 ...
 */
int cbtf_send(const unsigned size, const void* data)
{
    /* Send the data and inform the caller whether we succeeded or not */
    return CBTF_SendToFile(size, data);
}
